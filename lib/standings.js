...
export async function getStandings() {
  ...
  if (!season) return { season: null, first: [], second: [] };
  ...
  const points = await prisma.playerPoints.findMany({
  ...
  const matches = await prisma.match.findMany({
    ...
    select: { id: true, setsA: true, setsB: true, spectacleRating: true },
  });
  ...
  // points + matches played + spectacle
  ...
  const rows = players.map((p) => {
    ...
    return {
      ...
      played,
      setsWon: sets,
      spectacleAvg: sp != null ? Number(sp.toFixed(2)) : null,
      greens: green,
      reds: red,
      fairplay: green - red,
    };
